...
interface VocabCardEditorProps {
  onClose: () => void;
  t: (key: string, options?: { [key: string]: any }) => string; // Pass t function from parent (AIPanel)
}
...
const VocabCardEditor: React.FC<VocabCardEditorProps> = ({ onClose, t }) => {
  ...
  const handleSetDefault = () => {
    if (selectedTemplateId) {
      ...
      alert(t('vocabCardEditor.defaultTemplateUpdatedAlert'));
    }
  };
  ...
  const placeholderInfo = (
    <div className="text-xs text-gray-400 mt-2 p-2 bg-gray-900 rounded">
      <p className="font-bold">{t('vocabCardEditor.placeholdersTitle')}:</p>
      ...
      <div className="bg-gray-800 text-white rounded-lg p-6 max-w-2xl w-full flex flex-col max-h-[95vh]">
        ...
        <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-2">
                <button onClick={handleSetDefault} className="px-3 py-1 bg-sky-600 text-sm rounded hover:bg-sky-500">{t('vocabCardEditor.setDefaultButton')}</button>
                <button onClick={handleSaveTemplate} className="px-3 py-1 bg-blue-600 text-sm rounded hover:bg-blue-500">{t('vocabCardEditor.saveAsNewButton')}</button>
                <button onClick={() => setViewMode(viewMode === 'edit' ? 'preview' : 'edit')} className="px-3 py-1 bg-indigo-600 text-sm rounded hover:bg-indigo-500 w-24">
                  {viewMode === 'edit' ? t('vocabCardEditor.previewButton') : t('vocabCardEditor.editButton')}
                </button>
            </div>
        </div>
        ...
        <div className="flex-grow overflow-y-auto pr-2">
          {viewMode === 'edit' ? (
            <>
              <div className="mb-4">
                <label className="block text-lg font-semibold mb-1">{t('vocabCardEditor.cardFrontTemplateLabel')}</label>
              ...
              <div className="mb-4">
                <label className="block text-lg font-semibold mb-1">{t('vocabCardEditor.cardBackTemplateLabel')}</label>
            ...
            <>
              <div className="mb-4">
                <h3 className="block text-lg font-semibold mb-1 text-gray-400">{t('vocabCardEditor.cardFrontPreviewLabel')}</h3>
              ...
              <div className="mb-4">
                <h3 className="block text-lg font-semibold mb-1 text-gray-400">{t('vocabCardEditor.cardBackPreviewLabel')}</h3>
        ...
        <div className="flex justify-end gap-4 mt-6 pt-4 border-t border-gray-700">
          <button onClick={onClose} className="px-4 py-2 bg-gray-600 rounded-lg hover:bg-gray-500">{t('vocabCardEditor.cancelButton')}</button>
