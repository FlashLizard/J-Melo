    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-800 text-white rounded-lg p-6 max-w-2xl w-full flex flex-col max-h-[95vh]">
        ...
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold">{t('cardViewer.title')}</h2>
          {isEditing ? (
            <div className="flex gap-2">
              <button onClick={handleSave} className="px-3 py-1 bg-green-600 text-sm rounded hover:bg-green-500">{t('cardViewer.saveButton')}</button>
              <button onClick={handleCancel} className="px-3 py-1 bg-gray-600 text-sm rounded hover:bg-gray-500">{t('cardViewer.cancelButton')}</button>
            </div>
          ) : (
            <button onClick={() => setIsEditing(true)} className="px-3 py-1 bg-blue-600 text-sm rounded hover:bg-blue-500">{t('cardViewer.editButton')}</button>
          )}
        </div>
        ...
        <div className="flex-grow overflow-y-auto pr-2">
          {isEditing ? (
            <>
              <div className="mb-4">
                <label className="block text-lg font-semibold mb-1">{t('cardViewer.cardFrontLabel')}</label>
              ...
              <div className="mb-4">
                <label className="block text-lg font-semibold mb-1">{t('cardViewer.cardBackLabel')}</label>
            ...
            <>
              <div className="mb-4">
                <h3 className="block text-lg font-semibold mb-1 text-gray-400">{t('cardViewer.frontPreviewLabel')}</h3>
              ...
              <div className="mb-4">
                <h3 className="block text-lg font-semibold mb-1 text-gray-400">{t('cardViewer.backPreviewLabel')}</h3>
        ...
        <div className="flex justify-between items-center mt-6 pt-4 border-t border-gray-700">
          <button onClick={goToPrevViewerItem} className="px-4 py-2 bg-gray-600 rounded-lg hover:bg-gray-500">{t('cardViewer.previousButton')}</button>
          <span>{currentViewerIndex + 1} / {viewerItems.length}</span>
          <button onClick={goToNextViewerItem} className="px-4 py-2 bg-gray-600 rounded-lg hover:bg-gray-500">{t('cardViewer.nextButton')}</button>
        </div>

        <button onClick={closeViewer} className="w-full mt-4 px-4 py-2 bg-red-700 rounded-lg hover:bg-red-600">{t('cardViewer.closeViewerButton')}</button>
      </div>
    </div>
  );
};
