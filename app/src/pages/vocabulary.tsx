    ...
    <>
      ...
      <main className="bg-[#0f172a] min-h-screen text-white pb-12 selection:bg-indigo-500/30">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-10 flex flex-col h-[calc(100vh-2rem)] lg:h-[calc(100vh-4rem)]">
          {/* Header */}
          <header className="flex justify-between items-center gap-4 mb-8 bg-gray-800/40 p-4 sm:p-6 rounded-3xl border border-gray-700/50 shadow-lg backdrop-blur-sm flex-shrink-0">
            <div className="flex items-center gap-3 sm:gap-4 min-w-0">
              ...
              <h1 className="text-xl sm:text-3xl font-extrabold tracking-tight bg-gradient-to-br from-white to-gray-400 bg-clip-text text-transparent truncate">{t('vocabularyPage.title')}</h1>
            </div>
            <Link href="/" className="p-2.5 sm:p-3 bg-gray-700/80 text-gray-200 rounded-2xl hover:bg-gray-600 hover:text-white transition-all flex items-center justify-center border border-gray-600/50 shadow-sm flex-shrink-0" title={t('vocabularyPage.backToPlayer')}>
...
const WordCard = ({ word, song, t }: { word: WordRecord, song?: SongRecord, t: (key: string) => string }) => {
  ...
  return (
    <div 
        onClick={handleClick} 
        className={cn(
            "group bg-gray-800/60 backdrop-blur-sm p-4 rounded-2xl flex items-center gap-4 transition-all duration-300 border relative overflow-hidden", 
            isSelectionMode && selectedIds.has(word.id!) ? "border-indigo-500 ring-1 ring-indigo-500 bg-indigo-900/20 shadow-[0_0_15px_rgba(99,102,241,0.2)]" : "border-gray-700/50 hover:bg-gray-700 hover:border-gray-600 cursor-pointer shadow-md hover:shadow-xl hover:-translate-y-1"
        )}
    >
  ...
  return (
    <div className="bg-gray-800/40 backdrop-blur-sm rounded-3xl border border-gray-700/50 overflow-hidden mb-6 shadow-lg">
      <header 
        ...
        className="p-4 sm:p-5 flex items-center justify-between cursor-pointer hover:bg-gray-700/50 transition-colors border-b border-gray-700/30 group"
      >
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="flex items-center gap-4 min-w-0">
            <div className="w-12 h-12 rounded-xl bg-gray-900/50 flex items-center justify-center flex-shrink-0 shadow-inner border border-gray-700/50 group-hover:scale-105 transition-transform">
